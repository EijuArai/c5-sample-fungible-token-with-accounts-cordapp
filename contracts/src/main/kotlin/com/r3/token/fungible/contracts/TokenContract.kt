...
class TokenContract : FungibleContract() {
    ...
    class Issue : FungibleContractCreateCommand<Token>() {
        ...
        override fun onVerify(transaction: UtxoLedgerTransaction) {
            ...
            check(outputs.all { it.issuerAccount.participants.single() in transaction.signatories }) { CONTRACT_RULE_SIGNATORIES }
        }
    }
    ...
    class Transfer : FungibleContractUpdateCommand<Token>() {
        ...
        override fun onVerify(transaction: UtxoLedgerTransaction) {
            ...
            check(inputs.all { it.ownerAccount.participants.single() in transaction.signatories }) { CONTRACT_RULE_SIGNATORIES }
        }
    }
    ...
    class Redeem : FungibleContractDeleteCommand<Token>() {
        ...
        override fun onVerify(transaction: UtxoLedgerTransaction) {
            val inputs = transaction.getInputStates(Token::class.java)
            val signingKeys = inputs.flatMap { listOf(it.issuerAccount.participants.single(), it.ownerAccount.participants.single()) }
